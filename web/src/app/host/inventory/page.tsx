...
function InventoryContent() {
  ...
  return (
    <div className="min-h-screen flex bg-[#F8FAFC] dark:bg-[#070B14] text-gray-900 dark:text-gray-100">
      {/* ============================================================ */}
      {/* INVENTORY MANAGER SIDEBAR (MATCHING NAVBAR #2563EB)         */}
      {/* ============================================================ */}
      <aside
        className={`fixed top-0 bottom-0 left-0 z-30 w-72 bg-[#2563EB] border-r border-[#1D4ED8] text-white flex flex-col justify-between transition-transform duration-200 lg:translate-x-0 shadow-lg ${
      ...
      >
        <div className="flex flex-col h-full">
          ...
          {/* Nav Links */}
          <div className="p-3 border-b border-[#1D4ED8] space-y-1">
            <Link
              href="/admin"
              className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xs text-xs font-semibold text-white/90 hover:text-white hover:bg-white/15 transition-colors"
            >
          ...
          <div className="p-3 flex-1 overflow-y-auto space-y-3">
            ...
            <div className="relative">
              <Search className="absolute left-2.5 top-2.5 w-3.5 h-3.5 text-blue-200" />
              <input
                type="text"
                placeholder="Search stay..."
                value={searchPropertyQuery}
                onChange={(e) => setSearchPropertyQuery(e.target.value)}
                className="w-full pl-8 pr-3 py-1.5 text-xs rounded-xs border border-[#1D4ED8] bg-[#1D4ED8] text-white placeholder:text-blue-200 focus:outline-none focus:ring-1 focus:ring-white/40"
              />
            </div>
                ...
                <button
                  ...
                  className={`w-full text-left p-3 rounded-xs border text-xs transition-all cursor-pointer ${
                    selectedProperty?.id === p.id
                      ? "border-white bg-white text-[#2563EB] font-bold shadow-md"
                      : "border-[#1D4ED8] hover:bg-white/10 bg-[#1D4ED8]/60 text-white"
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <span className="truncate">{p.title}</span>
                    <ChevronRight className={`w-3.5 h-3.5 shrink-0 ${selectedProperty?.id === p.id ? "text-[#2563EB]" : "text-blue-200"}`} />
                  </div>
                  <span className={`text-[10px] font-normal block mt-0.5 ${selectedProperty?.id === p.id ? "text-[#2563EB]/80" : "text-blue-200"}`}>
