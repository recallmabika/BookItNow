...
export default function AdminDashboardPage() {
  ...
  async function handleAddRoomSubmit(e: React.FormEvent) {
    ...
    const token = localStorage.getItem("bookitnow_token");
    if (!token) {
      setRoomError("Authentication expired. Please log in again.");
      return;
    }

    if (!roomName.trim()) {
      setRoomError("Please provide a room type title (e.g. Luxury Chalet).");
      return;
    }

    if (roomBasePrice <= 0) {
      setRoomError("Base nightly price must be greater than $0.");
      return;
    }
    ...
    try {
      await addRoomType(
        ...
        {
          name: roomName.trim(),
          description: roomDesc.trim() || `Comfortable and spacious lodging room at ${targetProperty.title}.`,
          max_adults: Number(roomAdults),
          max_children: Number(roomChildren),
          total_rooms: Number(roomTotalCount),
          base_price_per_night: Number(roomBasePrice),
          currency: roomCurrency,
          amenities: ["Air Conditioning", "En-suite Bathroom", "Balcony / Patio", "Free WiFi"],
          photos: ["/hero-bg.jpg"]
        },
        token
      );
